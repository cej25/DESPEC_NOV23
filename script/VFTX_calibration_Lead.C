...
void run(){
  fin->cd("Histograms/Unpack/VFTX_0/FineTime/Leading/");
...
void VFTXCalibration(int module, int channel,bool plot =false) {
  ...
  TH1F* h = (TH1F*)gDirectory->Get(Form("VFTX0_FineTime_leading_ch%02d",channel));
  TCanvas *c;
  if(plot){
    c= new TCanvas();
    c->Divide(2,1);
  ...
  std::ofstream f_int;
  std::ofstream f_cal;
  f_cal.open(Form("LeadBin2Ps/VFTX_%05d_Bin2Ps_ch%02d.dat",VFTX_SN,channel),std::ios::out);
  f_int.open(Form("LeadBin2Ps/INT/VFTX_%05d_INT_ch%02d.txt",VFTX_SN,channel),std::ios::out);
