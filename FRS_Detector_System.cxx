...
void FRS_Detector_System::Clear_MTDC_32()
{
  for (int i=0; i<32; i++)
  {
...
void FRS_Detector_System::Clear_MQDC_32()
{
  for (int i=0; i<32; i++)
  {
    mqdc32_raw[i] = -9999;
    ...
    switch(psubevt->GetProcid())
    {
        case 30: // FRS User Crate
        {
            // CAEN V820
            {
                ...
                {
                    std::cout << "Error: ProcID 30 barrier missed!" << std::hex << *pdata << std::dec << std::endl;
                }
                else
                {
                    pdata++; len++;
                    ...
                    if (vme_type != 4)
                    {
                        std::cout << "Error: Scaler type missed match! GEO: " << vme_geo << " " << " type 4 != " << vme_type << std::endl;
                    }
                    pdata++; len++;
            ...
            while (len < lenMax)
            {
                // CAEN V775 and V785
                {
                    if (get_bits(*pdata, 16, 31) != 0xF520)
                    {
                        std::cout << "Error: ProcID 30 barrier missed! " << std::hex << *pdata << std::dec << std::endl;
                    }
                    else
                    {
                        ...
                        if (vme_type != 4)
                        {
                            std::cout << "Issue in unpacking ProcID 30, exiting!" << std::endl;
                            break;
                        }
                        pdata++; len++; i_word++;
                ...
                {   
                    ...
                    if (vme_type == 8)
                    {
                        ...
                        for (int i_word = 2; i_word <= words; i_word++)
                        {
                            ...
                            if (vme_type == 0 && in_event != 1)
                            {
                                std::cout << "Error: ProcID 10 MTDC type 0 without header (word " << i_word << " of " << words << "): " << std::hex << *pdata << std::dec << std::endl;
                            }
                            if (vme_type == 3 && in_event != 1)
                            {
                                std::cout << "Error: ProcID 10 MTDC type 3 without header (word " << i_word << " of " << words << "): " << std::hex << *pdata << std::dec << std::endl;
                            }
                            if (vme_type == 3 && in_event == 1)
                            {
                                // tdc trailer
                            ...
                            {
                                // error status
                                std::cout << "Error: ProcID 10 MTDC error data found (word " << i_word << " of " << words << "): " << std::hex << *pdata << std::dec << std::endl;
                            }
                            if (vme_type == 17)
                            {
                                ...
                                {
                                    std::cout << "Error: ProcID 10 MTDC strange end buffer header: " << vme_type << " " << vme_geo << " != " << vme_geoEnd << std::endl;
                                    pdata++; len++;
                                    break;
                                }
                            }
        ...
        case 20: // TPC crate
        {
                    ...
                    else
                    {
                        ...
                        if ((vme_nlw > 0) && vme_type == 2)
                        {
                            for (int i = 0; i < vme_nlw; i++)
                            {
                                ...
                                vme_type = getbits(*pdata, 2, 9, 3);
                                vme_chn = getbits(*pdata, 2, 1, 5);
                                vme_tpcs2[vme_geo][vme_chn] = getbits(*pdata, 1, 1, 12);
                                pdata++; len++; i_word++;
                            }
                        }
                        // "sanity check" at the end of V7x5 unpacking
                        vme_type = getbits(*pdata, 2, 9, 3);
                        if (vme_type != 4)
                        {
            ...
            // CAEN V1190
            {
                ...
                else
                {
                        ...
                        for (int i_word = 2; i_word <= words; i_word++)
                        {
                            ...
                            if (vme_type == 0 && in_event == 1)
                            {
                                ...
                                else
                                {
                                    if (value > 0)
                                    {
                                        // do trailing stuff - commented out in FRS code though
                                    }
                                }
                            }
                            if (vme_type == 0 && in_event != 1)
                            ...
                            {
                                // error status
                                std::cout << "Error: ProcID 20 MTDC error data found (word " << i_word << " of " << words << "): " << std::hex << *pdata << std::dec << std::endl;
                            }
                            if (vme_type == 17)
                            {
                                // extended trigger time tag, do nothing?
                            }
                            if (vme_type == 16)
                            {
                                Int_t vme_geoEnd = getbits(*pdata, 1, 1, 5);
                                if (vme_geo != vme_geoEnd)
                                {
                                    std::cout << "Error: ProcID 20 MTDC strange end buffer header: " << vme_type << " " << vme_geo << " != " << vme_geoEnd << std::endl;
                                    pdata++; len++;
                                    break;
                                }
                            }
                            ...
                            if (vme_type != 1 && vme_type != 0 && vme_type != 3 && vme_type != 4 && vme_type != 17 && vme_type != 16 && vme_type != 24)
                            {
                                std::cout << "Error: ProcID 20 MTDC strange type: " << vme_type << " (word " << i_word << " of " << words << "): " << std::hex << *pdata << std::dec << std::endl;
                            }
                            pdata++; len++;
                        }
                    }
                    ...
                    {
                        std::cout << "Error: ProcID 20 MTDC global header not found: " << vme_type << " ; data field: " << std::hex << std::setprecision(8) << *(pdata-1) << " " << *pdata << std::dec << std::endl;
        ...
        case 40: // VFTX at S2
        {
            // skip trivia and vetar information
            // vetar s2 removed for now: 17.11.23 -- Martin
            // VFTX
            {
                // first 4 bits are reserved for ID: in S2 case, its 0
                if (get_bits(*pdata, 4, 31) != 0xAB00000)
                {
                    std::cout << "Error: ProcID 40 barrier missed! " << std::hex << *pdata << std::dec << std::endl;
                }
                else if (get_bits(*pdata, 0, 4) != 0)
                {
                    std::cout << "Error: ProcID 40 VFTX ID should be 0, is: " << std::hex << get_bits(*pdata, 0, 4) << std::dec << std::endl;
                }
                else
                {
                    // skip next two words (always trigger window register and status register)
                    pdata += 3;
                    len += 3;
                    // now match MVLC stack header:
                    if (get_bits(*pdata, 16, 31) != 0xF500)
                    {
                        std::cout << "Error: ProcID 40 MVLC stack header missed! " << std::hex << *pdata << std::dec << std::endl;
                    }
                    else
                    {
                        Int_t words = get_bits(*pdata, 0, 15);
                        ...
                        for (int word = 0; word < words-1; word++)
                        {
                            ...
                            if (vftx_lead_mult[module][channel] < VFTX_MAX_HITS && !trailing)
                            {
                                ...
                                vftx_lead_mult[module][channel]++;
                            }
                            if (vftx_trail_mult[module][channel] < VFTX_MAX_HITS && trailing)
                            {
                                ...
                                //vftx_trail_mult[module][channel]++; // CEJ: is this line not needed? does not exist in FRS code
                            }
                        }
                    }
                }
            ...
            // Note: current data structure is VFTX --> MTDC --> MQDC
            {
                ...
                {
                    // header
                    no_of_words = *pdata & 0x00000FFF; // number of words from MTDC header
                    pdata++; len++;
                }
                else
                {
                    // if there is no data, don't show this message
                    if (num_data_words_from_MVLC != 0) 
                    {
                        std::cout << "Error: ProcID 40 MTDC header missed! " << std::hex << *pdata << std::dec << std::endl;
                    }
                }

                // data starts
                if (((*pdata & 0xFFC00000) >> 22) == 0b0000010000)
                {
                    Clear_MTDC_32(); // CEJ: added from TFRSUnpackEvent
                    ...
                    for (int i_wrd = 0; i_wrd < no_of_words - 1; i_wrd++)
                    {
                        ...
                        if (MTDC_trig_flg == 0)
                        {
                            mtdc32_dt_trg0_raw[MTDC_chnl_num] = MTDC_time_dif;
                        }
                        else if (MTDC_trig_flg == 1)
                        {
                            mtdc32_dt_trg1_raw[MTDC_chnl_num] = MTDC_time_dif;
                        }
                ...
                {
                    // if there is no data, don't show this word
                    if (num_data_words_from_MVLC != 0)
                    {
                        std::cout << "Error: ProcID 40 MTDC-32 trailer missed! " << std::hex << *pdata << std::dec << std::endl;
                    }
                }
            ...
            // MQDC-32
            {
                ...
                { 
                    ...
                    if (num_data_words_from_MVLC != 0)
                    {
                        std::cout << "Error: ProcID 40 MQDC header missed! " << std::hex << *pdata << std::dec << std::endl;
                    }
                }

                // data starts
                if (((*pdata & 0xFFE00000) >> 21) == 0b00000100000)
                {
                    Clear_MQDC_32();
                ...
                else
                {
                    ...
                    if (num_data_words_from_MVLC != 0)
                    {
                        std::cout << "Error: ProcID 40 MQDC-32 trailer missed! " << std::hex << *pdata << std::dec << std::endl;
                    }
                }
            } // end of MQDC-32
        } // end of ProcID 40
...
// added for some reason, whatever
Int_t FRS_Detector_System::get_bits(int value, uint32_t lo, uint32_t hi) {
